...
[CmdletBinding(DefaultParameterSetName='Parameter Set 1', 
    ...
    # Путь к OU В котором будет происходить поиск серверов для опроса 
        ...
        if ($OutputCsvFile -eq "") {
            $OutputCsvFile = "$serversfiledir\WinServerManagegment_Inv_$((Get-Date).ToString('dd-MM-yyyy')).csv"
            }    
    ...
    if (($CPUInfo.Name).count -ge 2) {$CPUInfoName=$CPUInfo.Name[0]}
        ...
        #$infoObject1 = $infoObject | Select-Object * -ExcludeProperty PSComputerName, RunspaceId, PSShowComputerName | Sort-Object servername #| `
        $infoObject = $infoObject | Select-Object * -ExcludeProperty PSComputerName, RunspaceId, PSShowComputerName | Sort-Object servername #| `
        ...
        Export-Csv -path $OutputCsvFile -NoTypeInformation -Encoding UTF8 -Delimiter ";" 
