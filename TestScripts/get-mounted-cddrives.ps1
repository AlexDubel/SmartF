    ...
    function Get-VMCDROM-Info {
        ...
        $MyExportarray = @()
            ...
            if ($kk) {
             ...
             #$result.Parent+=$kk.Parent
             #$result.ConnectionState+=$kk.ConnectionState
             #$result.Name+=$PSItem.Name
             #$result.Org+=$PSItem.Org
             #$result.OrgVdc+=$PSItem.OrgVdc
             #$result.Status+=$PSItem.Status
             #Write-Host kk=$kk
             #Write-Host result=$result
             #   Write-Host -ForegroundColor yellow "CIVMname="$PSItem.name"`t" -NoNewline;  
             #Write-Host -ForegroundColor yellow "ORGname="$PSItem.Org"`t" -NoNewline;  
             #Write-Host $PSItem | Get-VM
        ...
        $MyExportarray | Export-Excel -Path "c:\temp\cdrom-info.xlsx" -Title "Connected Cdrom to VM info." -AutoSize 
